...
(function () {
  ...
  function populateBookFilter() {
    // No visible book/testament <select> anymore — the Bible Navigation
    // modal handles that. This just keeps state.bookId valid.
    if (!BIBLE_DATA.books.find((b) => b.id === state.bookId)) {
      state.bookId = BIBLE_DATA.books[0] ? BIBLE_DATA.books[0].id : null;
    }
  }
  ...
  function renderChapter() {
    ...
    chapter.verses.forEach((verse) => {
      ...
      if (section) {
        const sectionTitle = (section.title && (section.title[versionId] || Object.values(section.title)[0])) || '';
        html += `<span class="section-heading">${escapeHtml(sectionTitle)}</span>`;
      }
