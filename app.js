...
(function () {
  ...
  /* ------------------------------------------------------------------ *
   *  READ VIEW — CHAPTER / VERSE PICKER SHEET
   * ------------------------------------------------------------------ */
  function openPickerSheet() {
    const book = findBook(state.bookId);
    if (!book) return;
    els.sheetBookName.textContent = book.name;
    els.chapterJumpInput.value = '';
    els.chapterJumpInput.max = String(book.chapters.length);
    renderChapterGrid(book);
    renderVerseGrid(book);
    els.versePickerOverlay.classList.remove('hidden');
    setTimeout(() => els.chapterJumpInput.focus(), 50);
  }
  function closePickerSheet() {
    els.versePickerOverlay.classList.add('hidden');
  }
  function renderChapterGrid(book) {
    els.chapterGrid.innerHTML = book.chapters
      .map(
        (c) =>
          `<button class="picker-cell${c.number === state.chapter ? ' current' : ''}" data-chapter="${c.number}">${c.number}</button>`
      )
      .join('');
  }
  function renderVerseGrid(book) {
    const chapter = book.chapters.find((c) => c.number === state.chapter);
    if (!chapter) {
      els.verseJumpSection.classList.add('hidden');
      return;
    }
    els.verseJumpSection.classList.remove('hidden');
    els.verseGrid.innerHTML = chapter.verses
      .map((v) => `<button class="picker-cell" data-verse="${v.number}">${v.number}</button>`)
      .join('');
  }
  ...
  els.chapterGrid.addEventListener('click', (e) => {
    const btn = e.target.closest('.picker-cell');
    if (!btn) return;
    const book = findBook(state.bookId);
    goToChapter(book.id, parseInt(btn.dataset.chapter, 10));
    renderChapterGrid(book);
    renderVerseGrid(book);
  });
  els.verseGrid.addEventListener('click', (e) => {
    const btn = e.target.closest('.picker-cell');
    if (!btn) return;
    closePickerSheet();
  ...
  els.chapterJumpForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const book = findBook(state.bookId);
    const n = parseInt(els.chapterJumpInput.value, 10);
    if (!book || !n || !book.chapters.find((c) => c.number === n)) {
      toast(`Enter a chapter between 1 and ${book ? book.chapters.length : 1}`);
      return;
    }
    goToChapter(book.id, n);
    els.chapterJumpInput.value = '';
    renderChapterGrid(book);
    renderVerseGrid(book);
  });
